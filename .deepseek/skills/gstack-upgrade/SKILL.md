...
```bash
if [ -d "$HOME/.deepseek/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.deepseek/skills/gstack"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".deepseek/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".deepseek/skills/gstack"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".deepseek/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".deepseek/skills/gstack"
elif [ -d "$HOME/.deepseek/skills/gstack" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.deepseek/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.deepseek/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.deepseek/skills/gstack"
  fi
fi
...
```bash
cd "$_ROOT"
git rm -r --cached .deepseek/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.deepseek/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.deepseek/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.deepseek/skills/gstack/` when you're ready."
...
.deepseek/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.deepseek/skills/gstack/` when you're ready."
