...
Read the `/office-hours` skill file at `$GSTACK_ROOT/office-hours/SKILL.md` using the file reader.
...
For LLM/prompt changes: check the "Prompt/LLM changes" file patterns listed in DEEPSEEK.md. If this plan touches ANY of those patterns, state which eval suites must be run, which cases should be added, and what baselines to compare against. Then use AskUserQuestion to confirm the eval scope with the user.
...
### TODOS.md updates
After all review sections are complete, present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `.deepseek/skills/review/TODOS-format.md`.
...
- If found, **replace it** entirely using the file editor. Match from \`## GSTACK REVIEW REPORT\`
