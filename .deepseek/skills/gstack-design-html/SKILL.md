...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.deepseek/skills/gstack/design/dist/design" ] && D="$_ROOT/.deepseek/skills/gstack/design/dist/design"
...
- If variant PNGs found: show them inline using the file reader.
...
2. If `$D` is not available, read the approved PNG inline using the file reader.
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.deepseek/skills/gstack/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/.deepseek/skills/gstack/design-html/vendor/pretext.js"
...
Write a single file using the file writer. Save to:
...
Show all three screenshots inline using the file reader. Check for:
...
- **Surgical edits in the refinement loop.** Use the file editor to make targeted changes,
  not the file writer to regenerate the entire file. The user may have made manual edits
