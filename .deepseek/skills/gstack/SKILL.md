...
- **Show screenshots:** After `$B screenshot`, `$B snapshot -a -o`, or `$B responsive`, always read this file on the output PNG(s) so the user can see them. Without this, screenshots are invisible.
...
`$B` is the browse binary (resolved from `$_ROOT/.deepseek/skills/gstack/browse/dist/browse` or `$GSTACK_ROOT/browse/dist/browse`).
