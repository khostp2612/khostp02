...
GSTACK_ROOT="$HOME/.deepseek/skills/gstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.deepseek/skills/gstack" ] && GSTACK_ROOT="$_ROOT/.deepseek/skills/gstack"
GSTACK_BIN="$GSTACK_ROOT/bin"
GSTACK_BROWSE="$GSTACK_ROOT/browse/dist"
GSTACK_DESIGN="$GSTACK_ROOT/design/dist"
_UPD=$($GSTACK_BIN/gstack-update-check 2>/dev/null || .deepseek/skills/gstack/bin/gstack-update-check 2>/dev/null || true)
...
# Check if DEEPSEEK.md has routing rules
_HAS_ROUTING="no"
if [ -f DEEPSEEK.md ] && grep -q "## Skill routing" DEEPSEEK.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
...
_VENDORED="no"
if [ -d ".deepseek/skills/gstack" ] && [ ! -L ".deepseek/skills/gstack" ]; then
  if [ -f ".deepseek/skills/gstack/VERSION" ] || [ -d ".deepseek/skills/gstack/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
Check if a DEEPSEEK.md file exists in the project root. If it does not exist, create it.

Use AskUserQuestion:

> gstack works best when your project's DEEPSEEK.md includes skill routing rules.
> This tells Claude to use specialized workflows (like /ship, /investigate, /qa)
> instead of answering directly. It's a one-time addition, about 15 lines.

Options:
- A) Add routing rules to DEEPSEEK.md (recommended)
- B) No thanks, I'll invoke skills manually

If A: Append this section to the end of DEEPSEEK.md:
...
Then commit the change: `git add DEEPSEEK.md && git commit -m "chore: add gstack skill routing rules to DEEPSEEK.md"`
...
`.deepseek/skills/gstack/`. Vendoring is deprecated. We will not keep vendored copies
up to date, so this project's gstack will fall behind.

Use AskUserQuestion (one-time per project, check for `~/.gstack/.vendoring-warned-$SLUG` marker):

> This project has gstack vendored in `.deepseek/skills/gstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .deepseek/skills/gstack/`
2. Run `echo '.deepseek/skills/gstack/' >> .gitignore`
3. Run `$GSTACK_BIN/gstack-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore DEEPSEEK.md && git commit -m "chore: migrate gstack from vendored to team mode"`
...
### B7. Update DEEPSEEK.md

First check: If DEEPSEEK.md already has a `## Testing` section → skip. Don't duplicate.
...
Only commit if there are changes. Stage all bootstrap files (config, test directory, TESTING.md, DEEPSEEK.md, .github/workflows/test.yml if created):
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.deepseek/skills/review/TODOS-format.md`).
...
Match against these patterns (from DEEPSEEK.md):
...
1. **Read DEEPSEEK.md** — look for a `## Testing` section with test command and framework name. If found, use that as the authoritative source.
2. **If DEEPSEEK.md has no testing section, auto-detect:**
...
Before proceeding, check DEEPSEEK.md for a `## Test Coverage` section with `Minimum:` and `Target:` fields. If found, use those percentages. Otherwise use defaults: Minimum = 60%, Target = 80%.
...
1. Read `.deepseek/skills/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.deepseek/skills/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
> You are classifying Greptile review comments for a /ship workflow. Read `.deepseek/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
Read `.deepseek/skills/review/TODOS-format.md` for the canonical format reference.
...
Co-Authored-By: DeepSeek <assistant@deepseek.com>
EOF
)"
```
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.deepseek/skills/gstack/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
> After completing the workflow, output a single JSON object on the LAST LINE of your response (no other text after it):
> `{"files_updated":["README.md","DEEPSEEK.md",...],"commit_sha":"abc1234","pushed":true,"documentation_section":"<markdown block for PR body's ## Documentation section>"}`
>
