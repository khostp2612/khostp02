...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.deepseek/skills/gstack/browse/dist/browse" ] && B="$_ROOT/.deepseek/skills/gstack/browse/dist/browse"
...
if [ ! -f ~/.gstack/projects/$SLUG/land-deploy-confirmed ]; then
  ...
  CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' DEEPSEEK.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
...
```bash
# Check for persisted deploy config in DEEPSEEK.md
DEPLOY_CONFIG=$(grep -A 20 "## Deploy Configuration" DEEPSEEK.md 2>/dev/null || echo "NO_CONFIG")
echo "$DEPLOY_CONFIG"
...
If `PERSISTED_PLATFORM` and `PERSISTED_URL` were found in DEEPSEEK.md, use them directly
...
and any persisted config from DEEPSEEK.md.
...
1. **DEEPSEEK.md persisted config:** Check for a staging URL in the Deploy Configuration section:
```bash
grep -i "staging" DEEPSEEK.md 2>/dev/null | head -3
```
...
**If C:** **STOP.** "Running `/setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to DEEPSEEK.md so I'll know exactly what to do next time. Run `/land-and-deploy` again when that's done."
...
Read DEEPSEEK.md to find the project's test command. If not specified, use `bun test`.
...
```bash
git diff --name-only $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main)...HEAD -- README.md CHANGELOG.md ARCHITECTURE.md CONTRIBUTING.md DEEPSEEK.md VERSION
```
...
If staging was detected in Step 1.5c (or from DEEPSEEK.md deploy config), and the changes
...
If a deploy status command was configured in DEEPSEEK.md (e.g., `fly status --app myapp`), use it instead of or in addition to GitHub Actions polling.
...
If DEEPSEEK.md has a custom deploy status command in the "Custom deploy hooks" section, run that command and check its exit code.
