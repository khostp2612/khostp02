...
**If prior retros exist:** Load the most recent one using the file reader. Calculate deltas for key metrics and include a **Trends vs Last Retro** section:
```
                    Last        Now         Delta
...
Use the file writer to save the JSON file with this schema:
```json
{
  "date": "2026-03-08",
  "window": "7d",
...
[ -z "$DISCOVER_BIN" ] && [ -x .deepseek/skills/gstack/bin/gstack-global-discover ] && DISCOVER_BIN=.deepseek/skills/gstack/bin/gstack-global-discover
...
If a matching prior retro exists, load it with the file reader. Show a **Trends vs Last Global Retro** table with deltas for key metrics: total commits, LOC, sessions, streak, context switches/day.
...
Use the file writer to save JSON to `~/.gstack/retros/global-${today}-${next}.json`:
...
- Do not read DEEPSEEK.md or other docs — this skill is self-contained
