...
description: |
  ...
  the configuration to DEEPSEEK.md so all future deploys are automatic.
...
checks, and deploy status commands — then persist everything to DEEPSEEK.md.

After this runs once, `/land-and-deploy` reads DEEPSEEK.md and skips detection entirely.
...
```bash
grep -A 20 "## Deploy Configuration" DEEPSEEK.md 2>/dev/null || echo "NO_CONFIG"
```

If configuration already exists, show it and ask:

- **Context:** Deploy configuration already exists in DEEPSEEK.md.
...
Read DEEPSEEK.md (or create it). Find and replace the `## Deploy Configuration` section
...
Saved to DEEPSEEK.md. /land-and-deploy will use these settings automatically.

Next steps:
- Run /land-and-deploy to merge and deploy your current PR
- Edit the "## Deploy Configuration" section in DEEPSEEK.md to change settings
...
- **DEEPSEEK.md is the source of truth.** All configuration lives there — not in a separate config file.
